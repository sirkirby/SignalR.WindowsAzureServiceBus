...
namespace SignalR.WindowsAzureServiceBus
{
	...
	public partial class ServiceBusMessageBus : IMessageBus, IDisposable
	{
		...
		public ServiceBusMessageBus(string topicPathPrefix, int numberOfTopics, string connectionStringName, string instanceId)
		{
		...
		void EnsureOpen()
		{
			lock (this.openMutex)
			{
				if (!this.isOpen)
				{
					...
					for (var i = 0; i < this.NumberOfTopics; i++)
					{
						this.EnsurePartitionTopicSubscription(i);
					}
		...
		void EnsurePartitionTopicSubscription(int topicNumber)
		{
			...
			try
			{
				...
				this.messagingFactories.Add(MessagingFactory.CreateFromConnectionString(connectionString));
